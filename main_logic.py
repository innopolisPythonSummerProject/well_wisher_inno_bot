...
from holiday_parameters import Holiday
...
async def delete_holiday(message: types.Message):
    ...
    if not holiday_name.strip():
        ...
        if result:
            ...
            await message.answer('Sorry, but there is no information about this holiday')
...
async def process_simple_calendar(callback_query: CallbackQuery, callback_data: dict):
    ...
    if selected:
        if holiday.is_birthday is True:
            await add_birthday_to_db(chat_id=chat_id, user=user, callback_query=callback_query, date=date)
        else:
            await add_holiday_to_db(chat_id=chat_id, holiday_name=holiday.holiday_name, callback_query=callback_query, date=date)
...
async def add_holiday_to_db(chat_id, holiday_name, callback_query, date):
    ...
    if results:
        ...
        update_query = (table.update().where(table.c.data == holiday_name).values(is_birthday=False, date=date))
        session.execute(update_query)
        ...
        insert_query = table.insert().values(data=holiday_name, is_birthday=False, date=date)
        session.execute(insert_query)
